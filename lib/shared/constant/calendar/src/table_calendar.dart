...
class TableCalendar<T> extends StatefulWidget {
  ...
  final CalendarBuilders<bool> calendarBuilders;
  ...
  TableCalendar({
    ...
    this.pageAnimationDuration = const Duration(milliseconds: 600),
...
class _TableCalendarState<T> extends State<TableCalendar<T>> {
  ...
  Widget _buildCell(DateTime day, DateTime focusedDay) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (!isDisabled) {
          final events = widget.eventLoader?.call(day) ?? false;
          Widget? markerWidget =
          ...
          if (events == true && markerWidget == null) {
            ...
            markerWidget = const Positioned(
              right: 4,
              bottom: 25,
              child: CircleAvatar(maxRadius: 5, backgroundColor: yellow),
            );
          }
  ...
  Widget _buildSingleMarker(DateTime day, bool event, double markerSize) {
